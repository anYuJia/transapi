...
function generateAuthUrl(port) {
  const params = new URLSearchParams({
    ...
    client_id: CLIENT_ID,
    prompt: 'consent',
    redirect_uri: `https://tunnel.mortis.edu.kg:${port}/oauth-callback`,
    response_type: 'code',
    scope: SCOPES.join(' '),
    state: STATE
  });
...
const server = http.createServer((req, res) => {
  ...
  const url = new URL(req.url, `https://tunnel.mortis.edu.kg:${port}`);
...
server.listen(0, () => {
  ...
  log.info(`服务器运行在 https://tunnel.mortis.edu.kg:${port}`);
